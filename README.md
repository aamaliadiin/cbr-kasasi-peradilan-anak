...
│
├── progress_checkpoint_cbr.txt    # Checkpoint halaman terakhir scraper (auto-generated)
├── riwayat_terunduh_cbr.txt       # Riwayat nomor perkara yang sudah diunduh (auto-generated)
└── logs/                          # Log preprocessing
```
...
Environment 2: env_cbr
    └── Untuk notebook: 03_case_retrieval + 04_case_reuse + 05_evaluation
...
| `05_evaluation.ipynb` | Python (CBR) |
...
Google Colab **hanya disarankan untuk notebook 02 sampai 05** (setelah PDF sudah ada di lokal dan diupload ke Google Drive).
...
*(Angka di bawah diambil langsung dari output eksekusi `05_evaluation.ipynb` pada test set 20% = 28 kasus; akan bervariasi sedikit jika notebook scraping dijalankan ulang karena dataset MA bertambah seiring waktu.)*
...
| SVM (TF-IDF, 4-fold CV) | 0.892 | — | — | 0.89 |
...
Brief tugas menyebut nama file seperti `03_retrieval.py` atau `04_predict.py`, namun seluruh implementasi menggunakan format **Jupyter Notebook (.ipynb)** yang lebih interaktif dan memudahkan debugging per-cell. Format ini sudah sesuai ketentuan karena brief juga menyebut *"notebook tahap retrieval"*.

File `.ipynb` tidak perlu diconvert ke `.py`. Untuk menjalankan notebook tanpa membuka browser Jupyter, gunakan perintah berikut dari terminal:

```bash
# Jalankan notebook dan simpan hasilnya
jupyter nbconvert --to notebook --execute 03_case_retrieval.ipynb

# Atau convert ke .py terlebih dahulu, lalu jalankan
jupyter nbconvert --to script 03_case_retrieval.ipynb
python 03_case_retrieval.py
```
